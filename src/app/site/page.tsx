    ...
    <main className="">
      <section className="h-full w-full pt-36 relative flex items-center justify-center flex-col">
        ...
        <div className="flex justify-center item-center relative md:mt-[-70px]">
          ...
          <div className="bottom-0 top-[50%] bg-gradient-to-t dark:from-background left-0 right-0
           absolute z-10"></div>
        </div>
      </section>
      <section className="flex item-center justify-center flex-col gap-4 md:!mt-20 mt-[-60px]">
        <h2 className="text-4xl text-center">Choose what fits you right</h2>
        <p className="text-muted-foreground text-center"> Our Straightforward pricing plans are tailored to meed your needs. If {" you're"}
          not <br/> ready to commit you can get started for free.
        </p>
        <div className="flex justify-center gap-4 flex-wrap mt-6">
          {pricingCards.map((card)=>(
            //WIP: wire up free product form stripe
            <Card key={card.title} className={clsx('w-[300px flex flex-col justify-between',{
              ...
              <CardFooter className="flex flex-col items-start gap-4">
                <div>{card.features.map((feature)=><div key={feature} className="flex gap-2
                items-center">
                    <Check/>
                    <p className="text-muted-foreground">{feature}</p>
                </div>)}</div>
                <Link href={`/agency?plan=${card.priceId}`} className={
                  clsx('w-full text-center bg-primary p-2 rounded-md',{'!bg-muted-foreground':card.title !== 'Unlimited Saas'})
                }
