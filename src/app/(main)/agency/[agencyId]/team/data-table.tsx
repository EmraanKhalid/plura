    ...
    return(
        <>
            <div className='flex items-center justify-between'>
                <div className='flex items-center p-y-4 gap-2'>
                    <Search/>
                    <Input value={(table.getColumn
                        (filterValue)?.getFilterValue() as string) ?? ''} placeholder='Search Name...' 
                        onChange={(event)=>{
                            table.getColumn(filterValue)?.setFilterValue(event.target.value)
                        }} className='h-12'/>
                </div>
                <Button className='flex gap-2' onClick={handleClick}>{actionButtonText}</Button>
            </div>
