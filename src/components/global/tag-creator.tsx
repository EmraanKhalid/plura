...
const TagCreator = ({ subAccountId, getSelectedTags, defaultTags }: Props) => {
    ...
    const handleTag = async () => {
        ...
        if (!selectedColor) {
            toast({
                variant: 'destructive',
                title: 'Tag need to have a color',
            })
            return
        }
        const tagData: Tag = {
        ...
        <AlertDialog>
            <Command className='bg-transparent'>
                ...
                <div className='relative'>
                    ...
                    <PlusCircleIcon size={20} onClick={handleTag}
                        className='absolute top-1/2 transform -translate-y-1/2 right-2
                hover:text-primary transition-all cursor-pointer text-muted-foreground'/>
                </div>
                <CommandList>
                    <CommandSeparator />
