...
const UserDetails = ({ id, type, userData, subAccounts }: Props) => {
    ...
    const onChangePermission = async (subAccountId: string, val:boolean,
    ...
    return (
        <Card className='w-full'>
            <CardHeader>
                ...
                <CardDescription>Add or Updat your Information</CardDescription>
                <CardContent>
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4'>
                            ...
                            />
                            <FormField disabled={form.formState.isSubmitting} control={form.control}
                            name="role" render ={({field})=>(<FormItem className='flex-1'>
                                <FormLabel>User Roler</FormLabel>
                                <Select disabled={field.value === 'AGENCY_OWNER'} onValueChange={(value)=>{
                                    if(value === "SUBACCOUNT_USER" || value  === "SUBACCOUNT_GUEST"){
                                        setRoleState("You need to have subaccount to assign SubAccount access to members.");
                                    }else{
                                        setRoleState("");
                                    }
                            ...
                            {authUserData?.role === "AGENCY_OWNER" && (
                                <div>
                                    ...
                                    <FormDescription className='mb-4'>
                                        You can give Sub Account access to team member by turning on access control
                                        for each Sub Account. This is only visible to Agency Owner.
                                    </FormDescription>
                                    <div className="flex flex-col gap-4">
                                        {subAccounts?.map((subAccount)=>{
                                            ...
                                            return (
                                                <div key={subAccount.id} className='flex flex-col 
                                                items-center justify-between rounded-lg border p-4'>
                                                    <div>
                                                        <p>{subAccount.name}</p>
                                                    </div>
                                                    <Switch disabled={loadingPermissions} checked={subAccountPermissionsDetails?.access}
